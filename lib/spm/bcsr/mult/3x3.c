/*
 * bcsr/mult/3x3.c -- BCSR 3x3 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_3x3) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
    ...
    for (i = r_start, i_ = r_start / 3; i < r_end; i += 3, i_++) {
        ...
        for (j = brow_ptr[i_], j_ = j / (3*3);
             j < brow_ptr[i_+1]; j += 3*3, j_++) {
            ...
            yr2 += bvalues[j+6]*x[x_start] + bvalues[j+7]*x[x_start+1] + bvalues[j+8]*x[x_start+2];
        }
...
void SPM_BCSR_MT_NAME(_multiply_3x3) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
