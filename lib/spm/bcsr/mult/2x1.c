/*
 * bcsr/mult/2x1.c -- BCSR 2x1 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_2x1) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
    ...
    for (i = r_start, i_ = r_start / 2; i < r_end; i += 2, i_++) {
        ...
        for (j = brow_ptr[i_], j_ = j / (2*1);
             j < brow_ptr[i_+1]; j += 2*1, j_++) {
...
void SPM_BCSR_MT_NAME(_multiply_2x1) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
