/*
 * bcsr/mult/6x1.c -- BCSR 6x1 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_6x1) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
    ...
    for (i = r_start, i_ = r_start / 6; i < r_end; i += 6, i_++) {
        ...
        for (j = brow_ptr[i_], j_ = j / (6*1);
             j < brow_ptr[i_+1]; j += 6*1, j_++) {
...
void SPM_BCSR_MT_NAME(_multiply_6x1) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
