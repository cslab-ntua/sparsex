/*
 * bcsr/mult/1x7.c -- BCSR 1x7 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_1x7)(void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
	...
	for (i = r_start, i_ = r_start; i < r_end; i += 1, i_++) {
		...
		for (j = brow_ptr[i_], j_ = j / (1*7);
		     j < brow_ptr[i_+1]; j += 1*7, j_++) {
			SPM_CRS_IDX_TYPE x_start = bcol_ind[j_];
			yr0 += bvalues[j]*x[x_start] + bvalues[j+1]*x[x_start+1] + bvalues[j+2]*x[x_start+2] + bvalues[j+3]*x[x_start+3] + bvalues[j+4]*x[x_start+4] + bvalues[j+5]*x[x_start+5] + bvalues[j+6]*x[x_start+6];
		}
...
void SPM_BCSR_MT_NAME(_multiply_1x7)(void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
