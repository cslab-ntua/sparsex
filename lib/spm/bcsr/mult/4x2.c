/*
 * bcsr/mult/4x2.c -- BCSR 4x2 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_4x2) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
    ...
    for (i = r_start, i_ = r_start / 4; i < r_end; i += 4, i_++) {
        ...
        for (j = brow_ptr[i_], j_ = j / (4*2);
            ...
            yr2 += bvalues[j+4]*x[x_start] + bvalues[j+5]*x[x_start+1];
            yr3 += bvalues[j+6]*x[x_start] + bvalues[j+7]*x[x_start+1];
        }
...
void SPM_BCSR_MT_NAME(_multiply_4x2) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
