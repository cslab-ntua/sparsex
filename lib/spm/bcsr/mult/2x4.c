/*
 * bcsr/mult/2x4.c -- BCSR 2x4 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_2x4) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
    ...
    for (i = r_start, i_ = r_start / 2; i < r_end; i += 2, i_++) {
        ...
        for (j = brow_ptr[i_], j_ = j / (2*4);
             j < brow_ptr[i_+1]; j += 2*4, j_++) {
            SPM_CRS_IDX_TYPE x_start = bcol_ind[j_];
            yr0 += bvalues[j]*x[x_start] + bvalues[j+1]*x[x_start+1] + bvalues[j+2]*x[x_start+2] + bvalues[j+3]*x[x_start+3];
            yr1 += bvalues[j+4]*x[x_start] + bvalues[j+5]*x[x_start+1] + bvalues[j+6]*x[x_start+2] + bvalues[j+7]*x[x_start+3];
        }
...
void SPM_BCSR_MT_NAME(_multiply_2x4) (void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
