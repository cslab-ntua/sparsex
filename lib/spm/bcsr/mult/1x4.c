/*
 * bcsr/mult/1x4.c -- BCSR 1x4 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_1x4)(void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
	...
	for (i = r_start, i_ = r_start; i < r_end; i += 1, i_++) {
		...
		for (j = brow_ptr[i_], j_ = j / (1*4);
		     j < brow_ptr[i_+1]; j += 1*4, j_++) {
...
void SPM_BCSR_MT_NAME(_multiply_1x4)(void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
