/*
 * bcsr/mult/1x2.c -- BCSR 1x2 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_1x2)(void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
	...
	for (i = r_start, i_ = r_start; i < r_end; i += 1, i_++) {
		...
		for (j = brow_ptr[i_], j_ = j / (1*2);
		     j < brow_ptr[i_+1]; j += 1*2, j_++) {
...
void SPM_BCSR_MT_NAME(_multiply_1x2)(void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
