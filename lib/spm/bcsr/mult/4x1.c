/*
 * bcsr/mult/4x1.c -- BCSR 4x1 multiplication method
 *
...
void SPM_BCSR_NAME(_multiply_4x1)(void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
	...
	for (i = r_start, i_ = r_start / 4; i < r_end; i += 4, i_++) {
		...
		for (j = brow_ptr[i_], j_ = j / (4*1);
		     j < brow_ptr[i_+1]; j += 4*1, j_++) {
...
void SPM_BCSR_MT_NAME(_multiply_4x1)(void *spm, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
