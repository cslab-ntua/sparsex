...
int main(int argc, char **argv)
{
    ...
    input_t *input = spx_input_load_mmf(argv[1]);
    ...
    vector_t *x = spx_vec_create_random(spx_mat_get_ncols(A), parts);
    vector_t *y = spx_vec_create_random(spx_mat_get_nrows(A), parts);
    vector_t *perm_x = spx_vec_create(spx_mat_get_ncols(A), parts);
    vector_t *perm_y = spx_vec_create(spx_mat_get_nrows(A), parts);
    vec_copy(x, perm_x);
    vec_copy(y, perm_y);
    ...
    matrix_t *B = spx_mat_tune(input, OP_REORDER);

    printf("Reordering vectors...\n");
    perm_t *p = spx_mat_get_perm(B);
