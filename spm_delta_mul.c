...
void SPMDELTA_NAME(_multiply)(void *matrix, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
	SPMDELTA_TYPE *delta = (SPMDELTA_TYPE *)matrix;
	...
	register const ELEM_TYPE *v_end = values + delta->nnz;
...
void SPMDELTA_NAME(_jmp_multiply)(void *matrix, VECTOR_TYPE *in, VECTOR_TYPE *out)
{
	SPMDELTA_TYPE *delta = (SPMDELTA_TYPE *)matrix;
	ELEM_TYPE *x = in->elements, *y = out->elements, *values = delta->values;
	register unsigned long y_indx=0;
