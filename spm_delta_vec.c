                                   ...
                                   unsigned char size)
{
	...
	elem_t *v, *v_cur;
	unsigned long i, delta_col, real_size;
	...
	*ctl_size = size;
	...
	//printf("DE: real_size:%lu rind:%lu\n", real_size, delta_state->row_indx);
	...
	for (i=0 ; i < real_size ; i++){
		v[i] = v_cur[i];
		//printf(">>>> values: %lu %lf\n", (v - delta->values + i), v[i]); 
	}
	parse_state->cur_unit_col += real_size;
	delta_state->val_indx += real_size;
	delta_state->col_indx += real_size;
}
                                 ...
                                 unsigned char size)
{
	...
	void *ci_deltas;
	elem_t *v_from, *v_to;
	unsigned long i, sp_idx, real_size, *ci_deltas_from;
	...
	*ctl_size = size;
	...
	ci_deltas = beanbag_alloc_nr(delta_state->ctl_bb,ci_deltas_size*real_size);
	...
	spm_delta_cisize_copy(ci_deltas, ci_deltas_from, real_size, sp_state->ci_max);
	delta_state->val_indx += real_size;
	sp_state->idx_consumed += real_size;
}
                              ...
                              delta_state_t *delta_state)
{
	...
	while ( len ){
		...
		if ( len >= SPM_DELTA_CTL_SIZE_MAX + 1){
			size = SPM_DELTA_CTL_SIZE_MAX;
			len -= (SPM_DELTA_CTL_SIZE_MAX+1);
		} else {
			size = len - 1;
			len = 0;
		}
...
static void delta_finalize_unit(parse_state_t *parse_state, void *delta_st)
{
	...
	assert(unit_len);
	//printf("delta_finilize_unit: row:%lu len:%lu colind:%lu \n",  parse_state->cur_row, unit_len, parse_state->cur_unit_col);
	if ( unit_len > UNIT_MIN_SEQ_LEN ) {
		...
		/*
		 * XXX : map values 0..255 to 1..256 for size  (aligned)
		 */
		//printf("*** unit_len:%lu\n", unit_len);
		while ( unit_len ){
			unsigned char size;
			if ( unit_len >= SPM_DELTA_CTL_SIZE_MAX + 1){
				size = SPM_DELTA_CTL_SIZE_MAX;
				unit_len -= (SPM_DELTA_CTL_SIZE_MAX+1);
			} else {
				size = unit_len - 1;
				unit_len = 0;
			}
			//printf("***\tsize:%u len:%lu\n", size, unit_len);
			delta_do_finalize_unit(parse_state, delta_state, size);
			delta_state->curval_indx += 1UL + size;
		}
	} else {
...
static spm_parser_t delta_vec_parser = {
...
spm_delta_t *spm_delta_vec_init_mmf(char *mmf_file, 
                                unsigned long *rows_nr, unsigned long *cols_nr,
                                unsigned long *nz_nr)
{
	int ret;
	...
	//delta = spm_parse_padd(f, *rows_nr, *cols_nr, *nz_nr, &delta_vec_parser, DMV_VEC_SIZE);
	delta = spm_parse_seq(f, *rows_nr, *cols_nr, *nz_nr, &delta_vec_parser);
	fclose(f);

	return delta;
}

void spm_delta_vec_multiply(spm_delta_t *delta, vector_t *in, vector_t *out)
{
	...
	for (;;) {
		...
		register unsigned long size = 1UL + uc_get(uc);
		...
		//printf(" y_indx: %lu x_indx: %lu ", y_indx, (unsigned long)(myx-x));
...
METHOD_INIT(spm_delta_vec_multiply, spm_delta_vec_init_mmf)
