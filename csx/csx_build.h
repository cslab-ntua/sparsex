...
double DoBuildSym(SparsePartitionSym<IndexType, ValueType> *spms_sym,
                  spm_mt *spm_mt, const CsxContext &csx_config);
template<typename IndexType, typename ValueType>
               ...
               const CsxContext &csx_config)
{
    ...
    // CSX JIT compilation
    Compile<SparsePartition<IndexType, ValueType> >(spm_mt, mt_context,
                                                    nr_threads,
                                                    csx_config.IsSymmetric());
                  ...
                  spm_mt *spm_mt, const CsxContext &csx_config)
{
    ...
    // CSX JIT compilation
    Compile<SparsePartitionSym<IndexType, ValueType> >(spm_mt, mt_context,
                                                       nr_threads,
                                                       csx_config.IsSymmetric());
    // Preprocessing finished; stop timer
    timer.Pause();
                   ...
                   const CsxContext &csx_config, double &time)
{
    ...
    spm_mt = PrepareSpmMt(csx_config); 
    time = DoBuild(spms, spm_mt, csx_config);
                      ...
                      const CsxContext &csx_config, double &time)
{
    ...
    time = DoBuildSym(spms_sym, spm_mt, csx_config);
