...
template<bool IsSym>
inline
spm_mt_t *SparseMatrix<InputPolicy>::CreateCsx(const CsxContext& csx_config,
                                               double &pre_time)
{
    spm_mt_t *spm = 0;

    try {
        spm = CreateCsx(csx_config, pre_time, internal::Sym<IsSym>());
                                               ...
                                               internal::Sym<false>)
{
    ...
    csx_ = BuildCsx<idx_t, val_t>(spi, csx_config, pre_time);
    delete spi;
    spi = 0;
                                               ...
                                               internal::Sym<true>)
{
    ...
    spms_sym = SparsePartitionSym<idx_t, val_t>::
        DoLoadMatrix(*this, rt_config.GetNrThreads());
    csx_ = BuildCsxSym<idx_t, val_t>(spms_sym, csx_config, pre_time);
