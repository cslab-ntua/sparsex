...
#include <sparsex/internals/CsxJit.hpp>
...
namespace sparsex {
  namespace csx {
    ...
    /**
     *  Routine responsible for retrieving the CSX or CSX-Sym sparse matrix
     *  format according to the command line parameters.
     *  
     *  @param mmf_fname    name of the sparse matrix file.
     *  @param spms         an initial sparse matrix format if it exists.
     *  @param spms_sym     an initial symmetric sparse matrix format if it
     *                      exists.
    ...
    /**
     *  Routine responsible for the Just-In-Time code generation.
     */
    template<typename InternalType>
    void GenerateCode(spm_mt_t *spm_mt,
		      vector<ThreadCtx<InternalType> > &data, 
     ...
     */
    template<typename IndexType, typename ValueType>
    void PreprocessThreadSym(ThreadCtx<SparsePartitionSym<IndexType,
			     ValueType> >& data);
    template<typename IndexType, typename ValueType>
    void PreprocessThread(ThreadCtx<SparsePartition<IndexType,
			  ValueType> > &data);
    ...
    (ThreadCtx<SparsePartition<IndexType, ValueType> > &data)
    {
      RtConfig &rtconfig = RtConfig::GetInstance();
      ...
      DrleMg = new EncodingManager<IndexType, ValueType>(data.GetSpm(),
							 rtconfig);
	...
	data.GetCsxManager()->MakeCsx(false);
    ...
    (ThreadCtx<SparsePartitionSym<IndexType, ValueType> >& data)
    {
      RtConfig &rtconfig = RtConfig::GetInstance();
		 ...
		 spm_mt *spm_mt)
    {
      ...
      vector<ThreadCtx<SparsePartition
      ...
      // Start preprocessing
      vector<shared_ptr<thread> > threads(nr_threads - 1);
      for (size_t i = 0; i < nr_threads - 1; ++i) {
        threads[i] =
	  make_shared<thread>(PreprocessThread<IndexType, ValueType>, 
			      ref(mt_context[i + 1]));
      }
      ...
      // CSX JIT compilation
      GenerateCode<SparsePartition<IndexType, ValueType> >
    ...
    template<typename IndexType, typename ValueType>
    spm_mt_t *BuildCsx
    (SparseInternal<SparsePartition<IndexType, ValueType> > *spms)
    {
      spm_mt_t *spm_mt;
      RtCtx &rt_context = RtCtx::GetInstance();
     ...
     spm_mt *spm_mt)
    {
      ...
      // Setup thread context
      vector<ThreadCtx<SparsePartitionSym
		       <IndexType, ValueType> > > mt_context(nr_threads);
      for (size_t i = 0; i < nr_threads; i++) {
        mt_context[i].SetId(i);
        mt_context[i].SetCpu(rt_context.GetAffinity(i));
      ...
      for (size_t i = 0; i < nr_threads - 1; ++i) {
        threads[i] =
	  make_shared<thread>(PreprocessThreadSym<IndexType, ValueType>, 
			      ref(mt_context[i + 1]));
      }
      ...
      // CSX JIT compilation
      GenerateCode<SparsePartitionSym<IndexType, ValueType> >
	(spm_mt, mt_context, nr_threads,
	 rt_config.GetProperty<bool>(RtConfig::MatrixSymmetric));
    }
