...
namespace sparsex {
  namespace io {
    ...
    template<typename IndexType, typename ValueType>
    ValueType CSR<IndexType, ValueType>::
    GetValue(IndexType row, IndexType col) const
    {
    ...
    // TODO: do binary search on column
    template<typename IndexType, typename ValueType>
    bool CSR<IndexType, ValueType>::
    SetValue(IndexType row, IndexType col, ValueType new_val)
    {
      for (IndexType i = rowptr_[row - 1] - !zero_based_;
	   i < rowptr_[row] - !zero_based_; i++) {
        IndexType j = colind_[i] - !zero_based_;
