...
namespace sparsex {
  namespace csx {
     ...
     *
     *  The statistics data collected during the preprocessing phase of CSX in
     *  order to determine the best matrix encodings. Three quantities are
     *  collected:
     *  
    ...
    public:
       ...
       *
       *  @return the number of encoded non-zero elements stored in this
       *          instance.
       */
     ...
     *
     *  This class represents the data structure used for collecting statistics
     *  data of type T. StatsCollection is essentially an accumulation tree 
     ...
     *
     *  Every level of the collection is associated with a statistics data 
     *  structure of type T, which must implement operator+(), operator==() 
     *  and a default constructor.
     *
     *  Every time new statistics data is added to the collection, the 
     *  intermediate nodes and their associated data are created or updated 
     *  accordingly.
     *
     *  Although iteration of the stored statistics data is supported, direct
     *  access to the internals of StatsCollection is not permitted whatsoever.
     *  However, there is a need to allow the manipulation of the collected
     *  statistics, e.g., cutting off instantiations with very low coverage
     *  of non-zero elements. This possible through the use of the so-called
     *  "statistics manipulators", which implement a variation of the visitor
     *  design pattern: StatsCollection iterates over its internal nodes and
     *  passes references of their data to the manipulator. The manipulator is
     *  then free to alter this data.
     *
     *  Statistics manipulation is possible at both the instantiation and the
     *  type level. At the type level a manipulator has access to all the stored
     *  instantiations of this type, but not to its associated statistics data
     *  structure; this is updated automatically by the StatsCollection in
     *  order to remain always consistent.
     *
    ...
    public:
       ...
       *
       *  If statistics data already exists for the inst instantiation, then 
       *  data will be accumulated to the statistics data of the already stored
       ...
       *
       *  If the requested type is not found in the other collection, then
       *  nothing happens. Otherwise, this collection's type statistics are
       *  copied from other replacing any data stored under the same type.
       *
       ...
       *  StatsManipulator::Manipulate method of the passed manipulator 
       *  depending on the level accessed:
       *  
       * - At the instantiation level, it is passed a reference to the stored
       *   instantiation data.
       * - At the type level, it is passed a reference to the entire type
       *   subtree.
       *   This allows certain manipulators (see BlockSplitter) to have a
       *   complete view of the encoded instantiations of a certain type,
       *   taking global decisions.
       *
       ...
       *
       *  @remark An instantiation or type is considered zeroed if it is equal
       *  to the default constructed statistics data type T.
       *
     ...
     *  process needs a broad look of all the encoded instantiations of a
     *  specific type (see BlockSplitter).
     *
     *  It is not necessary that both methods do useful work. Any method can
     *  simply return false to denote that it has not altered any data. In
     *  this case, the StatsCollection will not update the statistics data of
     *  its intermediate nodes, thus saving some computation time.
     *
    ...
    public:
      ...
      (Encoding::Type type, typename StatsCollection<T>::InstStats &stats) = 0;
    };
    ...
    /**
     *  @brief A simple statistics manipulator for cutting off instantiation
    ...
    /**
     *  @brief A statistics manipulator implementing the CSX split block
     *  technique.
     *
     *  The idea of the split block technique is to split larger blocks to
     *  smaller ones and merge them with existing dominant blocks in terms of
     *  non-zero coverage, so as to increase the total non-zero coverage of
     *  block patterns.
     *
     *  This manipulator searches for the first largest instantiation that
     *  exceeds a minimum threshold in non-zeros coverage and splits all
     *  larger blocks to a sum of this target instantiation and a remainder.
     *  This procedure continues until no more instantiations are left to split.
     *
