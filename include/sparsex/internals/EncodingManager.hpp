...
namespace sparsex {
  namespace csx {
     ...
     *
     *  This class is responsible for searching for patterns inside the matrix
     *  and encoding the matrix.
     */
    template<typename IndexType, typename ValueType>
    class EncodingManager
    {
    public:    
      static void CheckParams(RtConfig &config);
       ...
       *  @param stats the StatsCollection where the collected statistics will
       *  be stored
       */
      ...
      /**
       *  Instruct EncodingManager to ignore a specific type of pattern.
       *  Patterns of this type will not be considered during generation of
       *  statistics.
       *
      ...
      /**
       *  Choose a pattern type for encoding the matrix. This function chooses
       ...
       *  implements a heuristic to characterize the candidate encoding
       *  patterns. Statistics for the different patterns must have already
       *  been gathered. Currently, the heuristic implemented by this function
       *  just computes the following:
       *
       *  <br/><br/>
       *  <i>nr_nzeros_encoded - nr_patterns</i>
       *  <br/><br/>
       *
       *  This is actually a metric of the compression that the specified type
       *  of patterns can achieve.
       *
      ...
      /**
       *  Output the split points of the sorting windows, if windows are used
       *  for the encoding of the matrix.
       *
    ...
    private:
      ...
      /**
       *  Encode elements from a block when the split_blocks_ function is
       *  active.
       *
       *  @param row_no  row number of the encoded elements  
       *  @param xs      deltas of elements that are going to be encoded.
       ...
       *  the SPLIT_BY_NNZ policy is set, the window size is "rounded" to the
       *  next or previous full row, whichever is closer in terms of nonzero
       *  elements.
       */
    ...
    private:
      ...
      PreprocMethod preproc_method_;
      PreprocHeuristic preproc_heur_;
    ...
    CheckParams(RtConfig &config)
    {
      PreprocMethod method =
	config.GetProperty<PreprocMethod>(RtConfig::PreprocMethod);

      if (method.GetType() == PreprocMethod::FixedPortion) {
        double sampling_portion = config.GetProperty<double>
	  (RtConfig::PreprocSamplingPortion);
        size_t samples_max =
	  config.GetProperty<size_t>(RtConfig::PreprocNrSamples);
        if (samples_max <= 0) {
	  ...
	  exit(1);
        ...
        LOG_INFO << "Preproc method: fixed portion\n";
        LOG_INFO << "* Sampling portion: " << sampling_portion << "\n";
        LOG_INFO << "* Number of samples: " << samples_max << "\n";
      } else if (method.GetType() == PreprocMethod::FixedWindow) {
        size_t sort_window_size =
	  config.GetProperty<size_t>(RtConfig::PreprocWindowSize);
        size_t samples_max =
	  config.GetProperty<size_t>(RtConfig::PreprocNrSamples);
        if (samples_max <= 0) {
	  LOG_ERROR << "invalid number of samples\n";
	  ...
	  exit(1);
        ...
        LOG_INFO << "Preproc method: fixed window\n";
        LOG_INFO << "* Window size: " << sort_window_size << "\n";
        LOG_INFO << "* Number of samples: " << samples_max << "\n";
      }

      PreprocHeuristic heur = config.GetProperty<PreprocHeuristic>
	(RtConfig::PreprocHeuristic);
      LOG_INFO << "Compression heuristic: " << heur.GetName() << "\n";
      LOG_INFO << "Substruture types: "
	       << config.GetProperty<string>
	(RtConfig::PreprocXform) << "\n";
    }
    ...
    EncodingManager(SparsePartition<IndexType, ValueType> *spm,
      ...
      : spm_(spm),
	config_(config),
	min_limit_(config_.GetProperty<size_t>(RtConfig::MatrixMinUnitSize)),
	max_limit_(config_.GetProperty<size_t>(RtConfig::MatrixMaxUnitSize)),
	min_perc_(config_.GetProperty<double>(RtConfig::MatrixMinCoverage)),
	sort_window_size_(config_.GetProperty<size_t>(
						      RtConfig::PreprocWindowSize)),
	preproc_method_(config_.GetProperty<PreprocMethod>(
								 RtConfig::PreprocMethod)),
	preproc_heur_(config_.GetProperty<PreprocHeuristic>(
								  RtConfig::PreprocHeuristic)),
	sampling_portion_(config_.GetProperty<double>(
						      RtConfig::PreprocSamplingPortion)),
	samples_max_(config_.GetProperty<size_t>(RtConfig::PreprocNrSamples)),
	split_blocks_(config_.GetProperty<bool>(RtConfig::MatrixSplitBlocks)),
	onedim_blocks_(config_.GetProperty<bool>(RtConfig::MatrixOneDimBlocks)),
	split_type_(SPLIT_BY_NNZ)
    {
      ...
      if (preproc_heur_.GetType() == PreprocHeuristic::MinCost) {
        minimize_cost_ = true;
      } else {
    ...
    template<typename IndexType, typename ValueType>
    void EncodingManager<IndexType, ValueType>::
    GenAllStats(StatsCollection<StatsData> &stats)
    {
      encoded_inst_.clear();
    ...
    template<typename IndexType, typename ValueType>
    Encoding::Type EncodingManager<IndexType, ValueType>::
    ChooseType(const StatsCollection<StatsData> &stats)
    {
    ...
    template<typename IndexType, typename ValueType>
    unsigned long EncodingManager<IndexType, ValueType>::
    GetTypeScore(Encoding::Type type, const StatsData &sdata)
    {
    ...
    template<typename IndexType, typename ValueType>
    void EncodingManager<IndexType, ValueType>::
    EncodeSerial(const EncodingSequence &encseq)
    {
      if (!spm_->GetNrNonzeros())
        // If matrix is empty, just return
        return;
    ...
    template<typename IndexType, typename ValueType>
    void EncodingManager<IndexType, ValueType>::
    OutputSortSplits(ostringstream& out)
    {
	     ...
	     vector<Element<IndexType, ValueType> > &encoded)
    {
      ...
      FOREACH (const RLE<IndexType> &rle, rles) {
        ...
        rle_freq = rle.freq;
        if (rle_freq != 1 &&
	    encoded_inst_.find(make_pair(e.GetType(), rle.val)) !=
	    encoded_inst_.end()) {
	  col += rle.val;
	  if (col != rle.val) {
#if !SPX_USE_NUMA
    ...
    template<typename IndexType, typename ValueType>
    void EncodingManager<IndexType, ValueType>::
    DoEncodeBlock(IndexType row_no, vector<IndexType> &xs,
		  vector<ValueType> &vs,
		  vector<Element<IndexType, ValueType> > &encoded)
    {
      typename vector<ValueType>::iterator vi = vs.begin();
	  ...
	  nr_elem = 0;
        ...
        if (rle.val == 1 &&
            encoded_inst_.find(make_pair(e.GetType(), nr_elem/block_align)) !=
	    encoded_inst_.end() &&
    ...
    template<typename IndexType, typename ValueType>
    void EncodingManager<IndexType, ValueType>::
    DoEncodeBlockAlt(IndexType row_no, vector<IndexType> &xs,
		     vector<ValueType> &vs,
		     vector<Element<IndexType, ValueType> > &encoded)
    {
      typename vector<ValueType>::iterator vi = vs.begin();
    ...
    template<typename IndexType, typename ValueType>
    void EncodingManager<IndexType, ValueType>::
    UpdateStatsBlock(Encoding::Type type, vector<IndexType> &xs,
		     size_t block_align,
		     vector<Element<IndexType, ValueType> > &elems,
		     StatsCollection<StatsData> &stats)
    {
    ...
    DoCheckSortByNNZ()
    {
      assert(sort_window_size_ <= spm_->GetNrNonzeros() &&
	     "invalid sort window");
