...
namespace sparsex {
  namespace csx {
     ...
     *
     *  Marking elements can be useful when collecting statistics for the
     *  different encodings of the sparse matrix. For example, you can mark
     *  elements in order to treat them differently when generating or
     *  processing statistics. In order to keep the generic element's
     *  implementation as simple as possible, we introduce the notion of
     *  markers. Every element is associated with a marker, which we can use it
     *  to mark it. A marker can accommodate more than a single mark.
     *
     ...
     *  - PatternStart: this mark is set for element at the beginning of a
     *                  pattern.
     *
     *  @see EncodingManager
     */
     ...
     *  substructure instantiation. In this case, a generic element consists of
     *  the set of non-zero values belonging to the substructure and the
     *  coordinates (row, column) of the upper leftmost element of the 
     *  instantiation.
     *
    ...
    public:
       ...
       *  @param col the column of the upper leftmost element in the
       *             substructure.
       ...
       *  Less than operator.
       *
       *  The elements are compared lexicographically based on their
       *  coordinates, i.e., *this < rhs, if this->GetRow() < rhs.GetRow() ||
       *  (this->GetRow() == rhs.GetRow() && this->GetCol() < rhs.GetCol()).
       *
      ...
      operator=(const Element<IndexType, ValueType> &other)
      {
        row_ = other.row_;
        col_ = other.col_;
        size_ = other.size_;
       ...
       */
      Element<IndexType, ValueType> &
      operator=(Element<IndexType, ValueType>&& other)
      {
        row_ = other.row_;
        col_ = other.col_;
        size_ = other.size_;
    ...
    template<typename IndexType, typename ValueType>
    void swap(Element<IndexType, ValueType> &a,
	      Element<IndexType, ValueType> &b)
    {
      using std::swap;
      swap(a.row_, b.row_);
      swap(a.col_, b.col_);
    ...
    /**
     *  Get the last column of elem's non-zero values based on a specific 
     *  iteration order.
     *
     *  If the desired iteration order is different from elem's, then the
     *  elem.GetCol() is returned. Otherwise, the column of the last non-zero
     *  value in the elem's substructure is returned.
     *
