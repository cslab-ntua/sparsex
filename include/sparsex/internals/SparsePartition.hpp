...
namespace sparsex {
  namespace csx {
    ...
    /**
     *  Internal representation of a sparse matrix. This class can also 
     *  represent a sub-matrix of a larger one, starting at a specific row of
     *  the original matrix.
     */
    template<typename IndexType, typename ValueType>
    ...
    public:
      ...
      /**
       *  Extracts a window (sub-matrix) from this matrix. This methods copies
       *  the nonzero elements of the original matrix to the window.
       *
    ...
    /**
     *  Internal representation of a sparse symmetric matrix. This class can 
     *  also represent a sub-matrix of a larger one.
     */
    template<typename IndexType, typename ValueType>
    class SparsePartitionSym
    {
    private:
      ...
      SparsePartition<IndexType, ValueType> *m2_; // Matrix that contains the
    ...
    template<typename IndexType, typename ValueType>
    void TestMMF(SparsePartition<IndexType, ValueType> *sp,
		 const char *mmf_file);
    ...
    GetWindow(IndexType rs, IndexType length)
    {
      ...
      move(elems_.begin() + es, elems_.begin() + ee,
	   back_inserter(ret->elems_));
    ...
    PutWindow(SparsePartition<IndexType, ValueType> *window)
    {
	  ...
	  e.Transform(make_pair(static_cast<IndexType>(e.GetRow() + rs),
				e.GetCol()));
        }
      }
     ...
     */
    template<typename IndexType, typename ValueType>
    SparsePartition<IndexType, ValueType>::iterator::
    iterator(SparsePartition *sp, IndexType r_idx)
      : sp_(sp),
	row_idx_(r_idx),
    ...
    template<typename IndexType, typename ValueType>
    SparsePartition<IndexType, ValueType>::iterator::
    iterator(SparsePartition *sp, IndexType r_idx, struct end_tag)
      : sp_(sp),
	row_idx_(r_idx),
    ...
    template<typename IndexType, typename ValueType>
    bool SparsePartition<IndexType, ValueType>::iterator::
    operator==(const iterator &pi)
    {
      return !(*this != pi);
    }
    ...
    template<typename IndexType, typename ValueType>
    void SparsePartition<IndexType, ValueType>::iterator::
    operator++()
    {
     ...
     */
    template<typename IndexType, typename ValueType>
    void SparsePartitionSym<IndexType, ValueType>::
    DivideMatrix()
    {
